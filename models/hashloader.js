...
function resolveAfterDelay(ap_json, ba_json) {
  return new Promise(resolve => {
    setTimeout(() => {
      resolve(ap_json, ba_json);
    }, config.es.promiseDelay);
  });
}

async function asyncDelay(ap_json, query_object) {
  ...
  return await resolveAfterDelay(ap_json, ba_json);
}
...
function searchBaMatches(ApDocs){
  ApDocs.forEach(function(item,i){
    ...
    var obj = {};
    asyncDelay(ap_json, query_object);
